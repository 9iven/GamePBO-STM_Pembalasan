...
// Class utama pengendali game loop dan event listener
public class Game extends JPanel implements KeyListener, ActionListener {
    private final int UI_W = 280, GAME_W = 720, H = 450;

    // Agregasi objek komponen game
    ...
    // Manajemen State (kondisi game saat ini)
    ...
    public Game() {
        ...
        // Memuat posisi awal dan data musuh
        gameMap.loadLevelPosition(player);
        this.enemies = gameMap.getEnemiesForCurrentLevel();

        logMessage("Sistem Aktif.");
        logMessage("WASD: Bergerak");
        logMessage("SPASI: Interaksi");

        // Loop berjalan pada estimasi 60 Frame Per Second (16 ms)
    ...
    public void actionPerformed(ActionEvent e) {
        ...
        if (currentState == State.COMBAT_INPUT) {
            combatTimer -= 0.016; // Pengurangan waktu interogasi
            if (combatTimer <= 0) {
    ...
    protected void paintComponent(Graphics g) {
        ...
        // Memisahkan urusan rendering ke graphic engine agar class Game tetap ringkas
    ...
    public void keyPressed(KeyEvent e) {
        ...
        else if (currentState == State.COMBAT_RESULT && key == KeyEvent.VK_SPACE) currentState = State.EXPLORE;
    }
    ...
    private void handleExplore(int key) {
        ...
        if (dx != 0 || dy != 0) {
            ...
            // Validasi pengecekan posisi musuh agar tidak bisa ditembus
            boolean enemyBlock = false;
            for (Enemy enemy : enemies) {
                if (!enemy.isDefeated && enemy.x == nx && enemy.y == ny) {
                    enemyBlock = true;
                    break;
                }
            }
        ...
        if (key == KeyEvent.VK_SPACE) {
            ...
            // Logika transisi dan interaksi lingkungan
            if (gameMap.isInteractable(tx, ty, 3)) {
                ...
                if (gameMap.isGameWon()) currentState = State.WIN;
                    ...
                    gameMap.loadLevelPosition(player);
                    this.enemies = gameMap.getEnemiesForCurrentLevel();
                    logMessage("Naik ke area selanjutnya...");
                }
            }
                ...
                if (gameMap.currentLevel > 0) {
                    ...
                    gameMap.loadLevelPosition(player);
                    this.enemies = gameMap.getEnemiesForCurrentLevel();
                    logMessage("Kembali ke area sebelumnya.");
                }
            }
                ...
                if (player.currency >= 10 && player.getHp() < player.MAX_HP) {
                    ...
                    logMessage("Makan di Kantin! HP Penuh (-10 Koin)");
                } else if (player.getHp() >= player.MAX_HP) {
                    logMessage("Kenyang! HP Anda sudah penuh.");
                } else {
                    SoundManager.play("error");
                    logMessage("Koin tidak cukup! Butuh 10 Koin.");
                }
            }
            else if (gameMap.isInteractable(tx, ty, 5)) {
                if (player.getExperience() >= 50) {
                    player.setExperience(player.getExperience() - 50);
                    // Karena ini di dalam lingkup kelas Game, variabel akses dimanipulasi
                    player.maxCombatTime += 1.0;
                    player.setBaseDamage(player.getBaseDamage() + 10);
                    SoundManager.play("win");
                    logMessage("UPGRADE BERHASIL! Waktu & Damage bertambah.");
                } else {
                    SoundManager.play("error");
                    logMessage("EXP kurang! Butuh 50 EXP untuk upgrade.");
                }
            }
            ...
            for (Enemy enemy : enemies) {
                if (enemy.x == tx && enemy.y == ty && !enemy.isDefeated) {
                    ...
                    logMessage("Memulai argumentasi!");
                }
            }
        }
    }
    ...
    private void resolveCombat() {
        ...
        if (playerSelection == enemyAttack) {
            ...
            combatMessage = "Berhasil mematahkan argumen musuh!";
            activeEnemy.isDefeated = true; player.combo++;

            player.currency += 5;
            player.setExperience(player.getExperience() + activeEnemy.getExpReward());
            logMessage("Menang! +5 Koin, +" + activeEnemy.getExpReward() + " EXP");
            ...
            combatMessage = "Gagal! Musuh menyerang mental Anda (-" + damage + " HP)";
